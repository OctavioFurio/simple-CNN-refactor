...
{
	Eigen::MatrixXd convolutedInput = _padding >= 1 ? Convolution2D(input, _kernel, _padding) : Convolution2D(input, _kernel);
...
{
	// calcular o dE/dX (derivada parcial da funcao custo com relacao a entrada).
	// Usado para atualizar a proxima camada.
